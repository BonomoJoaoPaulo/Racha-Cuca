...
solucao(ListaSolucao) :- 
    ...
    exatamenteaEsquerda(moto(vermelho, _, _, _, _, _), moto(_, _, _, _, 10000, _), ListaSolucao),
    ...
    exatamenteaEsquerda(moto(_, _, _, _, _, guitarra), moto(_, _, _, ccc-3333, _, _), ListaSolucao),
    ...
    exatamenteaDireita(moto(_, _, _, ddd-4444, _, _), moto(_, _, _, _, _, serpente), ListaSolucao),

    %A moto Vermelha está em algum lugar à esquerda da moto do Túlio.
    aEsquerda(moto(vermelho, _, _, _, _, _), moto(_, tulio, _, _, _, _), ListaSolucao),

    %A moto de 40 mil km está exatamente à direita da moto Verde.
    exatamenteaDireita(moto(_, _, _, _, 40000, _), moto(verde, _, _, _, _, _), ListaSolucao),
    ...
    exatamenteaEsquerda(moto(_, _, _, _, _, fenix), moto(_, _, _, _, 30000, _), ListaSolucao),

    %A moto da montadora Japonesa está em algum lugar à direita da moto Vermelha.
    aDireita(moto(_, _, japonesa, _, _, _), moto(vermelho, _, _, _, _, _), ListaSolucao),
    ...
    exatamenteaDireita(moto(branco, _, _, _, _, _), moto(amarelo, _, _, _, _, _), ListaSolucao),
    ...
    aEsquerda(moto(verde, _, _, _, _, _), moto(_, fagner, _, _, _, _), ListaSolucao),
    aDireita(moto(verde, _, _, _, _, _), moto(_, _, _, _, _, guitarra), ListaSolucao),

    %A moto Amarela está ao lado da moto do Túlio.
    aoLado(moto(amarelo, _, _, _, _, _), moto(_, tulio, _, _, _, _), ListaSolucao),

    %A moto cuja placa é AAA-1111 está exatamente à direita da moto da montadora Chinesa.
    exatamenteaDireita(moto(_, _, _, aaa-1111, _, _), moto(_, _, chinesa, _, _, _), ListaSolucao),
